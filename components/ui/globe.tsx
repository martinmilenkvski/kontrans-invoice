...
import { Color, Scene, Fog, PerspectiveCamera, Vector3, MeshPhongMaterial } from "three";
...
function GlobeComponent({ globeConfig, data }: WorldProps) {
  ...
  const _buildMaterial = () => {
    ...
    const globeMaterial = globeRef.current.globeMaterial() as unknown as MeshPhongMaterial;
  ...
  const startAnimation = () => {
    ...
    globeRef.current
      .arcsData(data)
      .arcStartLat((d: any) => d.startLat as number)
      .arcStartLng((d: any) => d.startLng as number)
      .arcEndLat((d: any) => d.endLat as number)
      .arcEndLng((d: any) => d.endLng as number)
      .arcColor((d: any) => d.color as string)
      .arcAltitude((d: any) => d.arcAlt as number)
...
function CameraController({ globeConfig }: { globeConfig: GlobeConfig }) {
  ...
  useEffect(() => {
    ...
    if (globeConfig.initialPosition) {
      ...
      camera.position.x = cameraZ * Math.sin(phi) * Math.cos(theta);
      camera.position.y = cameraZ * Math.cos(phi);
      camera.position.z = cameraZ * Math.sin(phi) * Math.sin(theta);
      camera.lookAt(0, 0, 0);
    }
